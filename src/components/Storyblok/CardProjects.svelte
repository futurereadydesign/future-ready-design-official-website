<script>
  ...
  <article class="center">
    ...
    <div class="card" use:storyblokEditable={projects}>
      <!-- Toon de afbeelding van de projects -->
      <picture>
        <source srcset={projects.image?.filename} type="image/png">
        <img class="image-card" src={projects.image?.filename} alt={projects.image.alt} loading="lazy">
      </picture>
  
      <!-- Toon de titel van de projects -->
      <h2 class="title-card">{projects.title}</h2>
  
      <!-- Toon de datum van de projects -->
      <p class="date-card">{projects.date}</p>
  
      <!-- Voeg een link toe naar de projects door gebruik te maken van de slug -->
      <a href="/{slug}" use:storyblokEditable={projects} class="link">
