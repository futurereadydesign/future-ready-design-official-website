<script>
    ...
    <main in:fade={{duration: 300}} class="accessibility-landing header-margin">
        ...
        <form action="/accessibility/scan" method="get" class="url-input" bind:this={scanForm} on:keyup={onKeyInput}>
            <label for="site-url-input">{blok.site_url_input}</label>
            <input type="text" name="url" placeholder={blok.placeholder_input} autocomplete="off" id="site-url-input" required bind:this={scanInputField} on:focus={inputURLFocus} on:blur={inputURLBlur}>
        ...
        <section class="wrapper-small">
            <p>{@html renderRichText(blok.story)}</p>
        </section>

        <section class="wrapper-max section-block wcag no-overflow">
            {#each blok.wcag_checklist as wcag_checklist_content}
                <div class="section-block-inner">
                    <h2>{wcag_checklist_content.title}</h2>
                    <strong>{wcag_checklist_content.subtitle}</strong>
                    <a href="/accessibility/checklist" class="button button-secondary">
                        <span>{wcag_checklist_content.button}</span>
                    ...
                    <p>{@html renderRichText(wcag_checklist_content.information)}</p>
                </div>
                <div class="section-block-inner wcag-visual">
                    <img src={wcag_checklist_content.wcag_visual_1.filename} alt={wcag_checklist_content.wcag_visual_1.alt}>
                    <img src={wcag_checklist_content.wcag_visual_2.filename} alt={wcag_checklist_content.wcag_visual_2.filename}>
                </div>
            {/each}
        </section>
    </main>