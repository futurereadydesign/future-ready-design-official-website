...
// CHECK IF URL INPUT IS PRESENT ON PAGE 
    ...
    //     // Listen for keyboard and mouse events to determine the last interaction type
    ...
    //     // Apply custom focus style based on the last interaction type
    //     const input = document.getElementById('site-url-input');
    //     input.addEventListener('focus', function() {
    //         if (lastInteraction === 'keyboard') {
    //             input.classList.add('input-focus-visible');
    //         }
    //     });
    
    //     input.addEventListener('blur', function() {
    //         input.classList.remove('input-focus-visible');
    //     });
    // });
    ...
    // let scanButtonSubmitText = 'Doe de scan!'; // Default text

    // onMount(() => {
    //     const smallViewportSize = window.matchMedia("(max-width: 768px)");
    //     // Function to update button text based on viewport
    //     function updateButtonText(mediaQuery) {
    //         scanButtonSubmitText = mediaQuery.matches ? 'Scan!' : 'Doe de scan!';
    //     }

    //     // Update text on mount and on viewport change
    //     updateButtonText(smallViewportSize);
    //     smallViewportSize.addEventListener('change', () => updateButtonText(smallViewportSize));

    //     // Cleanup function to remove event listener when the component unmounts
    //     return () => {
    //         smallViewportSize.removeEventListener('change', () => updateButtonText(smallViewportSize));
    //     };
    // });
</script>
